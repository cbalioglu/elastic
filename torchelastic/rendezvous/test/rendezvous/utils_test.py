...
from torchelastic.rendezvous.utils import (
    _ClosableStore,
    _PeriodicTimer,
...
class UtilsTest(TestCase):
        ...
        for config_str in config_strs:
            with self.subTest(config_str=config_str):
                with self.assertRaisesRegex(
                    ValueError,
                    r"^The rendezvous config string must be in format <key1>=<value1>,...,<keyN>=<valueN>.$",
                ):
    ...
    def test_parse_rendezvous_raises_error_if_value_is_empty(self) -> None:
        ...
        for config_str in config_strs:
            with self.subTest(config_str=config_str):
                with self.assertRaisesRegex(
                    ValueError, r"^The 'a' rendezvous config must have a value specified.$"
                ):
                    _parse_rendezvous_config(config_str)
    ...
    def test_parse_rendezvous_endpoint_raises_error_if_hostname_is_invalid(self) -> None:
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                with self.assertRaisesRegex(
                    ValueError,
                    r"^The hostname of the rendezvous endpoint must be a dot-separated list of labels, an IPv4 address, or an IPv6 address.$",
                ):
    ...
    def test_parse_rendezvous_endpoint_raises_error_if_port_is_invalid(self) -> None:
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                with self.assertRaisesRegex(
                    ValueError,
                    r"^The port number of the rendezvous endpoint must be an integer between 0 and 65536.",
                ):
                    _parse_rendezvous_endpoint(endpoint, default_port=123)
...
class PeriodicTimerTest(TestCase):
    ...
    def test_cancel_stops_background_thread(self):
        ...
        self.assertTrue(any(t.name == "RepeatingTimer" for t in threading.enumerate()))

        timer.cancel()

        self.assertTrue(all(t.name != "RepeatingTimer" for t in threading.enumerate()))
    ...
    def test_timer_calls_background_thread_at_regular_intervals(self):
        ...
        self.assertGreaterEqual(
            ...
            f"The function has been called {call_count} time(s) but expected to be called at least {min_required_call_count} time(s).",
        )
        ...
        for actual_call_interval in actual_call_intervals:
            self.assertGreaterEqual(
                ...
                call_interval,
                f"The interval between two function calls was {actual_call_interval} second(s) but expected to be at least {call_interval} second(s).",
            )


class ClosableStoreTest(TestCase):
    ...
    def test_store_methods_raise_error_if_store_is_closed(self):
        ...
        with self.assertRaisesRegex(RuntimeError, r"^The store is already closed.$"):
            store.set("dummy", "dummy")
